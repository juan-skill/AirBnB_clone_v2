...
from fabric.api import local, env, run, put, sudo, lcd, cd
...
def do_clean(number=0):
    ...
    try:
        ...
        with lcd('versions'):
            if record_number == 0 or record_number == 1:
                local("ls -1t | tail -n +2 | xargs rm -rf")

            else:
                local("ls -1t | tail -n +{} | xargs rm -rf"
                      .format(record_number + 1))

        with cd('/data/web_static/releases/'):
            if record_number == 0 or record_number == 1:
                run("ls -1t | tail -n +2 | xargs rm -rf")
